...
class HomeController extends Controller
{
    ...
    public function customer_update_profile(Request $request)
    {
        ...
        if($request->hasFile('photo')){
            $fileName = "profileImage-" . time() . '.' . request()->photo->getClientOriginalExtension();
            $request->photo->storeAs('uploads/all/', $fileName);
    ...
    public function update_address(Request $request)
    {
        ...
        if($user->save()){
            flash(translate('Your Address has been updated successfully!'))->success();
            return back();
        }
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    public function search(Request $request, $category_id = null, $brand_id = null)
    {
        ...
        if($request->get('sort_pagination')){
            $paginate_value = $request->get('sort_pagination');
        }
        else{
            $paginate_value = 9;
        }
        $products = filter_products($products)->paginate($paginate_value)->appends(request()->query());
